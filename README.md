...
5. 必要であれば「ストップウォッチ」または「タイマー」をオプションで有効にし、時間を分単位（小数可、例: 1.5分）で指定する
   - テキストを入力している場合：画像サイズは自動で 720×368 になり、テキストの下に時計が別行で表示される
...
7. 「動画を生成する」ボタンを押すと動画が生成される
   ...
   - 時計オプションが有効かつテキスト入力ありの場合：720×368・1fpsで、実際に時間が進む動画（長さは指定分数+1秒）。ストップウォッチは `0:00` から指定時間まで、タイマーは指定時間から `0:00` までカウントする
   - 時計オプションが有効かつテキスト入力が空欄の場合：720×240・1fpsで、時計のみを中央に大きく表示した動画（長さは指定分数+1秒）
...
- ストップウォッチ／タイマーをオプションで追加可能（分単位で指定、表示は秒付き「分:秒」）
  - テキスト入力ありの場合：画像サイズが 720×368 になり、テキストの下に時計が表示される（368はH.264エンコーダーが要求する16の倍数の高さに合わせたもの。360だと一部端末でエンコードが失敗するため）
  - **テキスト入力が空欄の場合**：画像サイズは 720×240 のまま、時計のみを画面中央に大きく表示する（テキスト欄用のスペースを確保しないため）

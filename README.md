...
5. 必要であれば「ストップウォッチ」または「タイマー」をオプションで有効にし、時間を分単位（小数可、例: 1.5分）で指定する
   - **テキストを入力している場合**：画像サイズは 720×240 のまま、左480pxにテキスト、右240pxに時計が縦横中央で表示される（横並びレイアウト）
   - **テキスト入力欄が空欄の場合**：画像サイズは 720×240 のまま、時計だけが画面中央に大きく表示される（時計のみの動画を作りたい場合はテキストを空にする）
   - 表示形式は「分:秒」（例: `3:07`）。分が3桁（例: `123:45`）になっても、時計エリアの幅に収まるよう文字サイズが自動的に縮小されるため見切れない
6. プレビューで最終的な見た目を確認する
7. 「動画を生成する」ボタンを押すと動画が生成される
   - 時計オプションが「なし」の場合：これまで通り 720×240・1fps・1秒間（静止画1フレーム）の動画
   - 時計オプションが有効な場合：常に 720×240・1fpsで、実際に時間が進む動画（長さは指定分数+1秒）。ストップウォッチは `0:00` から指定時間まで、タイマーは指定時間から `0:00` までカウントする
     - テキスト入力ありの場合は左480px＝テキスト／右240px＝時計の横並び、テキスト入力なしの場合は時計のみを中央に大きく表示
8. 「ダウンロード」ボタンでファイルとして保存、または「共有」ボタンで端末の共有シートから共有できる
9. 対応端末では「ピクチャー・イン・ピクチャー」ボタンが表示され、押すと動画が小窓（フローティングウィンドウ）で再生され続ける。他のアプリを操作しながら動画・時計を確認したい場合に便利
...
- ストップウォッチ／タイマーをオプションで追加可能（分単位で指定、表示は秒付き「分:秒」）
  - 画像サイズは常に 720×240（時計オプションの有無・テキストの有無に関わらず変わらない）
  - **テキスト入力ありの場合**：左480pxにテキスト、右240pxに時計を縦横中央で表示する横並びレイアウト
  - **テキスト入力が空欄の場合**：時計のみを画面中央に大きく表示する
  - 時計の文字サイズは表示エリアの幅に収まるよう自動調整されるため、分が3桁になっても文字が見切れない
...
- **ピクチャー・イン・ピクチャー（PiP）対応**：生成した動画を小窓で表示したまま他の操作ができる。標準の`requestPictureInPicture()`とiPadOS Safari向けのレガシーAPI（`webkitSetPresentationMode`）の両方に対応しており、対応端末でのみボタンが表示される
...
- `sw.js` 自体は `navigator.serviceWorker.register('sw.js', { updateViaCache: 'none' })` で登録しています。この `updateViaCache: 'none'` は必須です。指定しないと、ブラウザのHTTPキャッシュに `sw.js` 本体が古いまま残ってしまい、`CACHE_VERSION` を上げても新しいService Worker自体が読み込まれない（＝アップデート通知が出ても「更新する」を押した後も反映されず、再度開くとまた通知が出る）という不具合が起きます。
